...
// - opts: { posId, orderType, isSelfPayment, memberCode, pagerNumber, roundingTolerance }
//
...
export function mapDoOrderPayload(cart = [], grossAmount = null, selectedMethod = 'qris', opts = {}) {
  ...
  let rounding = 0;
  if (grossAmount != null) {
    rounding = Math.round(toNumber(grossAmount) - computedTotal);
    if (Math.abs(rounding) > roundingTolerance) {
      // suspicious rounding — warn (do not throw to avoid breaking flow; caller may decide)
      // caller should validate before sending to do-order endpoint
      // you can change to `throw` if you want strict rejection
      // eslint-disable-next-line no-console
      console.warn(`[mapDoOrderPayload] suspicious rounding computed=${computedTotal} gross=${grossAmount} rounding=${rounding}`);
    }
  }
  ...
  const payload = {
    ...
    selfPaymentCategory,
    selfPaymentCode,
    selfPaymentRefId: '', // caller can set more descriptive ref if available
    idleTimeThreshold: 0,
    posId: posId,
    pagerNumber: pagerNumber != null ? String(pagerNumber) : null,
