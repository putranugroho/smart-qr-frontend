...
export function parseComboToMenuItem(combo) {
  ...
  const image = combo.imagePath || combo.image || (combo.comboGroups && combo.comboGroups[0] && combo.comboGroups[0].products && combo.comboGroups[0].products[0] && combo.comboGroups[0].products[0].imagePath) || "/images/no-image-available.jpg";
  ...
  if (Array.isArray(combo.comboGroups) && combo.comboGroups.length > 0) {
    price = combo.comboGroups.reduce((acc, g) => {
      ...
      const positive = prods.find(p => Number(p.maskingprice || 0) > 0);
      if (positive) return acc + (Number(positive.maskingprice || 0) * Number(positive.qty || 0));
      return acc + Number(prods[0]?.maskingprice || 0);
    }, 0);
