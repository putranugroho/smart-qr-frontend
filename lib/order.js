...
// const payload = await mapDoOrderPayload(cart, grossAmountFromMidtransOrNull, selectedMethod, { posId: 'MGI', orderType: 'DI', getUser: yourGetUserFunctionOrObject })
// NOTE: function is async now (to support async getUser)

export async function mapDoOrderPayload(cart = [], grossAmount = null, selectedMethod = 'qris', opts = {}) {
  // opts may include:
  // { posId, orderType, isSelfPayment, memberCode, tableNumber, roundingTolerance, getUser }
  // getUser may be:
  // - a function (sync or async) that returns an object { orderType, tableNumber, ... }
  // - an object that already contains orderType/tableNumber
  const {
    posId = 'MGI',
    orderType: optOrderType = null, // note: null means "not provided" so we can override from getUser
    memberCode = null,
    tableNumber: optTableNumber = null,
    roundingTolerance = 2000 // max allowed abs rounding before warn (adjust as needed)
  } = opts || {};

  // try to resolve getUser if provided
  let resolvedUser = null;
  try {
    const getUser = opts?.getUser;
    if (typeof getUser === 'function') {
      // allow sync or async function
      const maybe = getUser();
      resolvedUser = (maybe && typeof maybe.then === 'function') ? await maybe : maybe;
    } else if (getUser && typeof getUser === 'object') {
      resolvedUser = getUser;
    }
  } catch (e) {
    // swallow errors from getUser to avoid breaking payload building
    // you may want to log in development
    // console.warn('mapDoOrderPayload: getUser failed', e);
    resolvedUser = null;
  }

  // determine final orderType and tableNumber:
  // precedence:
  // 1) explicit opts.orderType / opts.tableNumber (if provided and not null)
  // 2) resolvedUser.orderType / resolvedUser.tableNumber (if available)
  // 3) fallback defaults
  const orderType = (optOrderType != null && optOrderType !== '') ? optOrderType
                    : (resolvedUser && (resolvedUser.orderType != null && resolvedUser.orderType !== '') ? resolvedUser.orderType : 'DI');

  const tableNumber = (optTableNumber != null && optTableNumber !== '') ? optTableNumber
                      : (resolvedUser && (resolvedUser.tableNumber != null && resolvedUser.tableNumber !== '' ) ? resolvedUser.tableNumber : '99');
  ...
  const payload = {
    ...
    taxes,
    posId: posId,
    tableNumber: tableNumber != null ? String(tableNumber) : String(tableNumber),
