...
function getItemOrderType(it) {
  if (it?.orderType) return it.orderType;
  if (Array.isArray(it?.menus) && it.menus[0]?.orderType) {
    return it.menus[0].orderType;
  }
  return "DI";
}
  ...
  */
  ...
  function signature(it) {
    ...
    if (it.type === 'combo' && Array.isArray(it.combos)) {
      if (it.clientInstanceId) return `combo_inst_${it.clientInstanceId}`;
      if (it.detailCombo?.clientInstanceId) return `combo_inst_${it.detailCombo.clientInstanceId}`;
    ...
    const orderType = getItemOrderType(it);

    return [
      String(it.productCode || ""),
      orderType,                        // ✅ PEMBEDA DI / TA
    ...
    if (it?.type === 'combo') {
      ...
      it.combos?.forEach(cb => {
        cb.products?.forEach(p => {
          ...
          // base product * qty product (dalam combo)
          const base = Number(p.price || 0) * pQty

          // condiments ikut qty condiment * qty product parent
          let condTotal = 0
