      ...
      it.menus.forEach((m) => {
        ...
        const menuTaxes = Array.isArray(m.taxes)
          ? m.taxes.map(t => {
              ...
              const taxableBase = (basePrice * qty) + condTotal; // condTotal already accounts for multiplyChildByParent
        ...
        const condsForPayload = conds.map(c => {
          const cQty = Math.max(1, toNumber(c.qty) || 1);
          const finalQty = multiplyChildByParent ? cQty * qty : cQty;
          const cPrice = toNumber(c.price);
          ...
          // pastikan pajak condiment juga ikut akumulasi
          taxes.forEach(t => accumulateTaxByName(t.taxName, t.taxAmount));
