  ...
  const hasMenusPayload = Array.isArray(it.menus) && it.menus.length > 0;
    ...
    if (Array.isArray(it.menus) && it.menus.length) {
      it.menus.forEach((m) => {
        ...
        // Condiments list (normalize)
        const conds = Array.isArray(m.condiments) ? m.condiments : [];

        // compute condTotal depending on multiplyChildByParent
        const condTotalPerMenu = conds.reduce((acc, c) => {
          const cQty = Math.max(1, toNumber(c.qty) || 1);
          const cPrice = toNumber(c.price);
          return acc + (cPrice * cQty);
        }, 0);

        // If condiment qty is per-menu and menu qty > 1, multiply:
        const condTotal = multiplyChildByParent ? condTotalPerMenu * qty : condTotalPerMenu;

        // add to subtotal
        subTotal += Math.round(basePrice * qty);
        subTotal += Math.round(condTotal);

        // taxes for menu (base + conds)
        const menuTaxes = Array.isArray(m.taxes)
          ? m.taxes.map(t => {
              const pct = toNumber(t.taxPercentage ?? 0);
              const taxableBase = Math.round(((basePrice * qty) + condTotal) * pct / 100); // condTotal already accounts for multiplyChildByParent
              const taxAmount = Math.round(basePrice * pct / 100);              
              accumulateTaxByName(t.taxName ?? t.name ?? '', taxableBase);
        ...
        // condiments payload: if multiplyChildByParent => multiply qty by menu qty
        const condsForPayload = conds.map(c => {
          const cQty = Math.max(1, toNumber(c.qty) || 1);
          const finalQty = 1;
          const cPrice = toNumber(c.price);

          // Condiment taxes: fallback ke cart original jika tidak ada taxes array
          const taxes = (Array.isArray(c.taxes) && c.taxes.length)
            ? c.taxes.map(t => {
                const pct = toNumber(t.taxPercentage || 0);
                const taxAmount = Math.round(cPrice * finalQty * pct / 100);
                const taxableBase = Math.round(cPrice * (multiplyChildByParent ? cQty * qty : cQty) * pct / 100);    
                return {
                  taxName: String(t.taxName ?? t.name ?? ''),
                  taxPercentage: pct,
                  taxAmount
            ...
            : (c.taxAmount ? [{
                ...
                taxAmount: Math.round(c.taxAmount * finalQty / (c.qty || 1))
              }] : []);
        ...
        menus.push({
          ...
          detailMenu: {
            ...
            itemName: m.detailMenu?.itemName ?? m.detailMenu?.title ?? '',
    ...
    if (!hasMenusPayload && it.type !== 'combo') {
      ...
      const condTotalPerMenu = conds.reduce((acc, c) => acc + toNumber(c.price), 0);
      subTotal += (basePrice + condTotalPerMenu) * qty;
    ...
    it.combos.forEach(cb => {
      ...
      if (Array.isArray(cb.products)) {
        cb.products.forEach(p => {
          ...
          const pQtyRaw = Math.max(1, toNumber(p.qty) || 1);
          // decide final product qty:
          const finalProductQty = multiplyChildByParent ? pQtyRaw * itemQty : pQtyRaw;

          // === SUBTOTAL ===
          subTotal += Math.round(basePrice * finalProductQty);

          // === PRODUCT TAX ===
          const productTaxes = Array.isArray(p.taxes)
            ? p.taxes.map(t => {
                const pct = toNumber(t.taxPercentage || 0);
                const taxAmount = Math.round(basePrice * finalProductQty * pct / 100);
          ...
          const condiments = Array.isArray(p.condiments)
            ? p.condiments.map(c => {
                const cPrice = toNumber(c.price);
                const cQtyRaw = Math.max(1, toNumber(c.qty) || 1);
                const finalCQty = multiplyChildByParent ? cQtyRaw * itemQty : cQtyRaw;

                // add to subtotal using finalCQty
                subTotal += Math.round(cPrice * finalCQty);

                const taxes = Array.isArray(c.taxes)
                  ? c.taxes.map(t => {
                      const pct = toNumber(t.taxPercentage || 0);
                      const taxAmount = Math.round(cPrice * finalCQty * pct / 100);
                      accumulateTaxByName(t.taxName ?? t.name ?? '', taxAmount);
          ...
          productsPayload.push({
            ...
            image: p.image ?? '',
            price: basePrice,
            qty: finalProductQty, // <-- use computed final qty
            taxes: productTaxes,
            condiments
          });
        });
